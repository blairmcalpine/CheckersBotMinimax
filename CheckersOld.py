...
class Board:
    def __init__(self, close_side, board=[]):
    ...
    def copy(self):
        ...
        return Board(self.__close_side, board=new_board)
    def destroy(self):
        for row in self.__board:
            for value in row:
                if isinstance(value, Checker):
    ...
    def move(self, x, y, ydir, xdir):
        ...
        if isinstance(checker, Checker):
            ...
            print('Selected Piece Not a Checker')
            return False
        if colour != self.__turn:
            print('Other Players Turn')
            return False
        if ((ydir == 'D' and colour == self.__close_side and not is_king) or
            (ydir == 'U' and colour == self.__far_side and not is_king)):
            print('Cannot go that direction')
            return False
        if ((xdir == 'L' and x == 0 and y%2) or
            (xdir == 'R' and x == 3 and not y%2) or
            (ydir == 'U' and y == 0) or
            (ydir == 'D' and y == 7)):
            print('Out of bounds')
            return False
        ...
        desired_location = self.__board[dir_dict_1[ydir]][dir_dict_1[xdir]]
        if isinstance(desired_location, Checker) and desired_location.get_colour() == colour:
            print('Trying to move onto friendly checker located at: '+str(dir_dict_1[xdir])+', '+str(dir_dict_1[ydir]))
            return False
        required_moves = []
        if self.__last_move != None:
            ...
            if required_moves and (x != self.__last_move[0] or y != self.__last_move[1]
                                   or not ydir+xdir in required_moves):
                print('Required double jump must be made')
                return False
        if isinstance(desired_location, type(None)):
            all_pieces = self.__get_colour(colour)
            for ix, iy in all_pieces:
                if self.__can_jump(ix, iy):
                    print('Required single jump must be made')
                    return False
